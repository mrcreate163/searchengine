...
Быстрый справочник для развертывания Search Engine на Timeweb Cloud.
...
### 1. Создать облачный сервер на Timeweb Cloud
```bash
# Параметры:
# - OS: Ubuntu 22.04 LTS
# - Тариф: S-2 (2GB RAM, рекомендуется) или S-1 (1GB RAM)
...
### Месячные расходы
- **Timeweb Cloud S-2 (2GB):** 499₽/месяц (~$5.5/месяц)
- **Timeweb Cloud S-1 (1GB):** 169₽/месяц (~$1.9/месяц, минимум)
- **Домен на Timeweb (.ru):** ~100₽/год (~$0.10/месяц)
- **Домен на Timeweb (.com):** ~500₽/год (~$0.60/месяц)
...
- Можно начать с S-1 (1GB) и увеличить позже
- Почти в 2 раза дешевле Beget при той же конфигурации
