...
Быстрый справочник для развертывания Search Engine на Beget VPS.
...
# Параметры:
# - OS: Ubuntu 22.04 LTS
# - Тариф: Virtual-2 (2GB RAM, рекомендуется) или Virtual-1 (1GB RAM)
# - SSH ключ: добавить при заказе
```
...
### Месячные расходы
- **Beget VPS (Virtual-2, 2GB):** 900₽/месяц (~$10/месяц)
- **Beget VPS (Virtual-1, 1GB):** 350₽/месяц (~$4/месяц, минимум)
- **Домен на Beget (.ru):** ~100₽/год (~$1/месяц)
- **Домен на Beget (.com):** ~600₽/год (~$0.70/месяц)
- **SSL сертификат:** 0₽ (Let's Encrypt)

**Итого:** от 360₽/месяц до 920₽/месяц

### Экономия
- Скидки при оплате от 6 месяцев на Beget
- Можно начать с Virtual-1 (1GB) и увеличить позже
- Первый месяц часто со скидкой для новых клиентов

## 🔗 Полезные ссылки

- [Полное руководство по развертыванию](DEPLOYMENT_GUIDE.md)
- [Beget: База знаний](https://beget.com/ru/kb)
- [Beget: VPS/VDS](https://beget.com/ru/kb/vps)
